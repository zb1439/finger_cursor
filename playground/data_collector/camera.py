...
class CollectingCamera(DefaultCamera):
    def capture_callback(self):
        def func(img, frame_count, img_path, label_path):
            print('Capturing image', frame_count)
            print('Saving to', img_path)
            img_name = os.path.join(img_path, str(frame_count)+'.png')
            ...
            with open(os.path.join(label_path, str(frame_count)+'.json'), 'w') as f:
